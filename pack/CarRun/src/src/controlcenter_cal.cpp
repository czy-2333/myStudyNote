...
public:
    ...
    vector<POINT> centerEdge_racing;    // 用于追逐区的中间赛道中心点集
    ...
    void controlCenterCal(TrackRecognition &track)
    {
        ...
        if (track.pointsEdgeLeft.size() > 45 && track.pointsEdgeRight.size() > 45)  //判断直道
        {
            //中线曲线拟合
            if(racingDetection.track_control)
            {
                if(racingDetection.sideLeft)  //左半赛道
                {
                    centerEdge_racing = bezier_curve_fitting(track.pointsEdgeLeft, track.pointsEdgeRight);
                    centerEdge = bezier_curve_fitting(track.pointsEdgeLeft, centerEdge_racing);
                }
                else  //右半赛道
                {
                    centerEdge_racing = bezier_curve_fitting(track.pointsEdgeLeft, track.pointsEdgeRight);
                    centerEdge = bezier_curve_fitting(centerEdge_racing, track.pointsEdgeRight);
                }
            }
            else if(dangerDetection.track_control)
            {
                if(dangerDetection.sideLeft)
                {
                    centerEdge_racing = bezier_curve_fitting(track.pointsEdgeLeft, track.pointsEdgeRight);
                    centerEdge = bezier_curve_fitting(track.pointsEdgeLeft, centerEdge_racing);
                }
