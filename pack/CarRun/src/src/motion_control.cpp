...
public:
    ...
    struct Params 
    {
        ...
        bool RescueEnable = false;
        bool dangerEnable = false;
        bool RacingEnable = false;
        string pathModel = "res/model/yolov3_mobilenet_v1";
        NLOHMANN_DEFINE_TYPE_INTRUSIVE(
            Params, speed_min, speedLow, speedHigh, speedAI, speedRing, speedCross, speedcoiled, speed_and_angle_k1, speed_and_angle_k2, 
            ...
            rowCutUp, rowCutBottom,  Debug, Button, SaveImage, CloseLoop, RingEnable, CrossEnable, StopEnable, 
            BridgeEnable, pathModel, K_foreword, Control_foreword_up, crossRec,
            Control_foreword_down, K_K_limit, point1_x, point1_y, point2_x, point2_y, point3_x, point3_y, Kp_speed, Ki_speed,
            Kd_speed, Kp_current, Ki_current, Kd_current, ParkingEnable, RescueEnable, dangerEnable, RacingEnable); // 添加构造函数
    };
    ...
    void speedControl(ControlCenterCal control) 
    {
        ...
        /*if(speed - motorSpeed > 0.15f)
            motorSpeed += 0.15f;
        else */if(speed - motorSpeed > 0.05f)
            motorSpeed += 0.05f;
            else if(speed - motorSpeed < -0.1f)
            motorSpeed -= 0.1f;
            else 
            motorSpeed = speed;
        // else if(speed - motorSpeed < -0.5f)
        //     motorSpeed -= 0.5f;
