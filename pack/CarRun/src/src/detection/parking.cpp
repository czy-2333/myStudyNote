...
public:
     ...
     */
    void parkingChack(vector<PredictResult> predict)
    {
        switch (step)
        {
        case Step::init: // 初始化：起点斑马线屏蔽
            ...
            {
                if (predict[i].label == LABEL_CROSSWALK) // AI识别标志
                {
        ...
        case Step::det: // AI未识别
            for (int i = 0; i < predict.size(); i++)
            {
                if (predict[i].label == LABEL_CROSSWALK) // AI识别标志
                {
                    if ((predict[i].y + predict[i].height) > ROWSIMAGE * 0.5) // 标志距离计算
                    {
                        countRec++;
                        step = Step::stop;   // 停车太晚时打开注释
                        break;
                    }
                }
            }
